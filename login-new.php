...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
   ...
   if ($suser && $spass) { // if all form items have been submitted
      ...
      $checksql = "SELECT * FROM $tbl_name WHERE ( mb_username = '".$suser."' OR mb_emailaddress = '".$suser."')";
      ...
      while($rs = mysqli_fetch_array($checkrs))
      {
         ...
         if ($checkactivate == 0) {
            ...
            if ($spass != $rs['mb_password']) {
               ...
               $zuser = stripslashes($zuser);
               $hour = time() + 7200;
               setcookie("ID_my_site", $zuser, $hour);
               setcookie("Key_my_site", $scode, $hour); 

               $lastlogin = date('Y-m-d H:i:s');
               $lastloginsql = "UPDATE $tbl_name SET mb_lastlogin='$lastlogin' WHERE ( mb_username = '".$suser."' OR mb_emailaddress = '".$suser."')"; 
  ...
  <body>
    <div class="LoginHolder">
      <div class="whitebox login">
        <h1>HelloBank</h1>
        <form action="logintest.php" method="post" name="myForm" id="myForm">

<?php if($error !="") { ?>
<div><p style="color:red;"><?php echo $error; ?></p></div>
<?php } ?>

          <input type="text" name="username" placeholder="username or email" id="username" value="<?php if (isset($_POST['username'])) echo $_POST['username'];?>">
          <img src="user.png">
